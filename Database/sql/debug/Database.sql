...
BEGIN
    RAISERROR(N'You cannot deploy this update script to target WIN-TN8FQU62P16. The database for which this script was built, DB_NMGSKL, does not exist on this server.', 16, 127) WITH NOWAIT
    RETURN
END

GO

IF (@@servername != 'WIN-TN8FQU62P16')
BEGIN
    RAISERROR(N'The server name in the build script %s does not match the name of the target server %s. Verify whether your database project settings are correct and whether your build script is up to date.', 16, 127,N'WIN-TN8FQU62P16',@@servername) WITH NOWAIT
    RETURN
END
...
CREATE TABLE [dbo].[tmp_ms_xx_T_PM_UserInfo] (
    ...
    [lcode]         UNIQUEIDENTIFIER CONSTRAINT [DF_T_PM_UserInfo_lcode] DEFAULT (newid()) NULL,
...
GO
PRINT N'Creating [dbo].[T_PM_UserInfo].[lcode].[MS_Description]...';


GO
EXECUTE sp_addextendedproperty @name = N'MS_Description', @value = N'��¼��', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'T_PM_UserInfo', @level2type = N'COLUMN', @level2name = N'lcode';
